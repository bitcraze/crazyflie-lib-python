...
# max_ROM = 1
max_ROM = 0.52   # change this variable according to the selected movement
ori_pos = 0.33    # original leg's sensor height
...
## (1) KnF: Knee Flex (drone: up/down)
## (2-1) HAR: Hip Abd (drone: Rightward-up/Leftward-down**; **for right leg;)
## (2-2) HAL: Hip Abd (drone: Leftward-up/Rightward-down**; **for left leg;)
## (3) HE: Hip Ext (drone: Backward-up/Forward-down)
## (4) HF: Hip Flex (drone: Forward-up/Backward-down)
## (5) HtT: Heel-to-toe (drone: keep forward step-by-step)
...
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
        ...
        # winsound.PlaySound('correct-6033.mp3', winsound.SND_FILENAME)
        print("start!!!")
        ...
        for i in range(1,11):
            ...
            # cv2.waitKey(500)   # showing white background for 5 second
            ...
            print("start moving down")
            mc.start_down(velocity=task_Vel)  # drone starts moving up
            time.sleep(0.1)

            while position_estimate_1[2] > start_pos_d:

                print("haven't reached the start point")

                while event3.is_set()==True and position_estimate_1[2] > start_pos_d:  # the subject doesn't follow the drone
                    print("!!!!!!")
                    mc.stop()
                    time.sleep(0.1)
            ...
            if position_estimate_1[2] < start_pos_d:
                under_dist = start_pos_d - position_estimate_1[2] 
                print("under the lower limit_2 (m): ", under_dist)
                mc.up(under_dist, velocity=under_dist/0.2)  # moving up to the start_pos_d within 0.2 second
                # time.sleep(0.1)
...
def drone_guide_mc_HAR(scf, event1, event2, event3): 
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
        ...
        for i in range(1,11):
            ...
            while not event2.is_set(): # the current leg sensor's position hasn't reached the max ROM in z-axis
                ...
                print("moving right-up 5 cm/s")
                # mc.start_up(velocity=task_Vel)
                # time.sleep(0.05/task_Vel)    # This line can't be blank!!!
                mc.move_distance(0.0, -0.05, 0.05, velocity=task_Vel)  # optional for the above two lines

                if position_estimate_1[2] - start_pos_d > move_dist:
                    over_dist = position_estimate_1[2] - (start_pos_d + move_dist) 
                    print("over the upper limit (m): ", over_dist)
                    mc.move_distance(0.0, over_dist, -over_dist, velocity=over_dist/0.1)  # moving back to the start_pos_d within 0.1 second
                    # time.sleep(0.5)
            ...
            move_down_dist = position_estimate_1[2] - start_pos_d
            print("move down distance (m): ", move_down_dist)
            mc.move_distance(0.0, move_down_dist, -move_down_dist, velocity=move_down_dist/2)  # moving back to the start_pos_d within 2 second
            print("Ready?")
            mc.stop()
...
def drone_guide_mc_HAL(scf, event1, event2, event3): 
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
...
def drone_guide_mc_HE(scf, event1, event2, event3): 
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
...
def drone_guide_mc_HF(scf, event1, event2, event3): 
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
...
def drone_guide_mc_HtT(scf, event1, event2, event3): 
# default take-off height = 0.3 m, take-off velocity = 0.2
    with MotionCommander(scf) as mc:
