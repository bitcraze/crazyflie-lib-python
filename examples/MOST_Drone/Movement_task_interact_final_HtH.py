...
def drone_guide_pc_HtH(scf, event1, event2, event3): 
    ...
    with PositionHlCommander(
        ...
        for i in range(1,11):
            ...
            while position_estimate_1[0] < move_dist:

                print("move forward 5 cm")
                pc.move_distance(dx, 0.0, 0.0)
                time.sleep(dx/task_Vel + 0.05)
...
def position_state_change(event1, event2, event3):
    ...
    while not event1.is_set():  # the drone hasn't finished the guiding yet
        ...
        if position_estimate_2[2] < max_ROM: # subject hasn't reached the max ROM yet
            ...
            if abs(abs(position_estimate_2[2] - ori_pos)-abs(position_estimate_1[2] - start_pos_d)) < 0.02:  # subject follows the drone
                # print("good job")
                event3.clear()
