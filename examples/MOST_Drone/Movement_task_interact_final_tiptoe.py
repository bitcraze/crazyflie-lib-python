...
max_ROM = 0.4   # change this variable according to the selected movement
...
def drone_guide_pc_tiptoe_for(scf, event1, event2): 
    ...
    with PositionHlCommander(
        ...
        for i in range(1,11):
            ...
            print("Please hold on for 3 second")
            time.sleep(3)

            
            ## Return process
            print("move down-backward")
            ...
            print("Please hold on for 1 second")
            time.sleep(1)
...
def position_state_change(event1, event2, event3):
    ...
    while not event1.is_set():  # the drone hasn't finished the guiding yet
        ...
        if abs(position_estimate_1[2] - (start_pos_d + move_dist)) < 0.03:

            # If the current leg sensor's position reaches the max ROM in z-axis
            if abs(position_estimate_2[2] - max_ROM) < 0.03: # subject hasn't reached the max ROM yet
        ...
        elif abs(position_estimate_1[2] - start_pos_d) < 0.03:
            # If the current leg sensor's position reaches the original position in z-axis
            if abs(position_estimate_2[2] - ori_pos) < 0.03: # subject hasn't returned to the start point yet
                # print("target reached!")
                event2.set()
