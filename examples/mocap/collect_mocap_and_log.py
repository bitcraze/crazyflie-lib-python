...
The actual data logged depends on the configuration of the micro-SD card logging deck. A properly configured micro-SD
card must be inserted into the logging deck to specify what data to log.
    ...
    def _on_packet(self, packet):
        ...
        if self.body_name not in self.qtm_6DoF_labels:
            ...
            if self.on_pose:
                ...
                self.on_pose([x, y, z, rot, timestamp])
...
def save_extpose_rot_matrix(cf, x, y, z, rot, timestamp):
    """
...
def get_random_pose_params():
    x = random.uniform(-0.5, 0.5)
    y = random.uniform(-0.5, 0.5)
    z = random.uniform(1, 1.5)
    yaw = random.uniform(0, 2*math.pi)
    time = random.uniform(2, 3)
    return x, y, z, yaw, time


def gen_random_spiral_params():
    angle = random.uniform(0, 2*math.pi)
    radius_0 = random.uniform(0.4, 0.8)
    radius_1 = random.uniform(0.4, 0.8)
    ascent = random.uniform(-0.5, 0.5)
    time = random.uniform(2, 3)
    clockwise = random.choice([True, False])
    return angle, radius_0, radius_1, ascent, time, clockwise
...
def run_sequence(cf, trajectory_id, duration):
    ...
    time.sleep(duration)
    commander.spiral(3.14/4, 1, 0.5, 0.5, 1, False, False)
    time.sleep(1)
    commander.spiral(3.14/4, 0.5, 1.0, 0.5, 1, False, True)
    time.sleep(1)
    commander.spiral(3.14/2, 1.0, 1.0, -1., 2, False, True)
    for _ in range(10):
        x, y, z, yaw, t = get_random_pose_params()
        commander.go_to(x, y, z, yaw, t)
        time.sleep(t)
        angle, radius_0, radius_1, ascent, t, clockwise = gen_random_spiral_params()
        commander.spiral(angle, radius_0, radius_1, ascent, t, False, clockwise)
        time.sleep(t)
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        if LOGGING:
            ...
            qtm_wrapper.on_pose = lambda pose: save_extpose_rot_matrix(
                cf, pose[0], pose[1], pose[2], pose[3], pose[4])
        ...
        cf.param.set_value('activeMarker.back', 0)
        ...
        # sleep to check yaw drift / convergence
        time.sleep(10.0)
