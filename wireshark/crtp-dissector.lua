...
function format_address(buffer)
    addr = buffer(1, 5):bytes():tohex()
    port = buffer(6, 1):uint()
...
function handle_setpoint_highlevel(tree, receive, buffer, channel, size)
    ...
    elseif cmd == Commands.COMMAND_GO_TO then
        ...
        if receive == 0 then
            group_mask = buffer(10, 1):uint()
            relative = buffer(11, 1):uint()
            x = buffer(12, 4):le_float()
            y = buffer(16, 4):le_float()
            z = buffer(20, 4):le_float()
            yaw = buffer(24, 4):le_float()
            duration = buffer(28, 4):le_float()
        end
    elseif cmd == Commands.COMMAND_START_TRAJECTORY then
        ...
        if receive == 0 then
            group_mask = buffer(10, 1):uint()
            relative = buffer(11, 1):uint()
            reversed = buffer(12, 1):uint()
            id = buffer(13, 1):uint()
            timescale = buffer(14):float()
        end
    ...
    elseif cmd == Commands.COMMAND_TAKEOFF_2 then
        ...
        if receive == 0 then
            group_mask = buffer(10, 1):uint()
            height = buffer(11, 4):le_float()
            yaw =  buffer(15, 4):le_float()
            use_yaw = buffer(19, 1):uint()
            duration = buffer(20, 4):le_float()
        end
...
function handle_parameter_port(tree, buffer, channel, size)
     ...
     if (channel == 1 or channel == 2) and size > 2 then
        ...
        -- Add variable id
        local var_id = buffer(9, 2):le_uint()
        local port_tree = tree:add(crtp, port_name)
        port_tree:add_le(f_crtp_parameter_varid, var_id)

        -- Add value
        if size > 3 then
            port_tree:add_le(f_crtp_parameter_val_uint, buffer(12):le_uint())
            port_tree:add_le(f_crtp_parameter_val_int, buffer(12):le_int())
        end
        if size >= 7 then
            port_tree:add_le(f_crtp_parameter_val_float, buffer(12):le_float())
        end
    end
end
...
function crtp.dissector(buffer, pinfo, tree)
    ...
    local header = bit.band(buffer(8, 1):uint(), 0xF3)
    ...
    -- Rest is CRTP packet
    local crtp_size = buffer:len() - 8
    subtree:add_le(f_crtp_size, crtp_size)

    -- Check for safelink packet
    if crtp_size == 3 and header == 0xF3 and buffer(9, 1):uint() == 0x05 then
    ...
    if crtp_port == Ports.Console then
        local port_tree = tree:add(crtp, port_name)
        port_tree:add_le(f_crtp_console_text, buffer(9):string())
    end
